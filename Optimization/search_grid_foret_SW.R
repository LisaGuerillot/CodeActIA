...
varied_hyperparameters <- crossing(resampling_frequency, immobility_threshold, immobility_length_threshold, no_labelled_threshold, time_threshold, tree_number, min_motion_time, window_second_length)
search_grid <- c()
for (i in 1:length(varied_hyperparameters$resampling_frequency)){
  res <- global_forest_with_SW(left_wrist, right_wrist, resampling_frequency = varied_hyperparameters$resampling_frequency[i], immobility_threshold = varied_hyperparameters$immobility_threshold[i], immobility_length_threshold = varied_hyperparameters$immobility_length_threshold[i], no_labelled_threshold = varied_hyperparameters$no_labelled_threshold[i], time_threshold = varied_hyperparameters$time_threshold[i], min_motion_time = varied_hyperparameters$min_motion_time[i], window_second_length = window_second_length, tree_number = varied_hyperparameters$tree_number[i])
  print(res)

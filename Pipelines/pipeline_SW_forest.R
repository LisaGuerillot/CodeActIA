...
database <- read.csv(file = file3, sep = ",", header = FALSE, col.names = c("Time", "Xright", "Yright", "Zright",
                                                                            "NormRight", "Xleft", "Yleft", "Zleft", 
                                                                            "NormLeft", "label", "idSubject", 
                                                                            "idMove", "id"), skip = 1)
...
predicted_labels <- data.frame(filtered_data$new_time_axis, filtered_data$overall_left_acc, 
                               filtered_data$overall_right_acc, filtered_data$start_mov, 
                               lab_or_no_lab, name_lab, confidence_level)
...
for (k in 1 : min(length(ind_beg_seg_filter), length(ind_end_seg_filter))){
  ...
  for (i in seq(1, length(left_segment_data), by=width_sliding_window/2)){
    ...
    descriptors <- compute_descriptors_window(left_window_data, right_window_data, 
                                              width_sliding_window, descriptors_database, criteria = "forest")
    if (!(TRUE%in%is.na(descriptors))){
      label_confidence_level <- classify_forest_without_SW(descriptors_database, descriptors,
                                                           no_labelled_threshold, tree_number)
...
colnames(predicted_labels) <- c("time", "left_acceleration", "right_acceleration", "start_mov", 
                                "lab_or_no_lab", "name_lab", "confidence_level")

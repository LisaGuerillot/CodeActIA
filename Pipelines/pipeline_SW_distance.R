...
immobility_threshold <- 0.2
immobility_length_threshold <- 1
labelled_max_distance <- 4
...
normalized_descriptors_database <- data.frame(label_movements, left_mean, right_mean, left_median, right_median, 
                                              left_var, right_var, left_Q1, right_Q1, left_Q3, right_Q3, left_kurto, 
                                              right_kurto, left_skew, right_skew)
...
for (k in 1 : min(length(ind_beg_seg_filter), length(ind_end_seg_filter))){
  ...
  for (i in seq(1, length(left_segment_data), by=width_sliding_window/2)){
    ...
    descriptors <- compute_descriptors_window(left_window_data, right_window_data, width_sliding_window, 
                                              descriptors_database, criteria = "distance")
    if (!(TRUE%in%is.na(descriptors))){
      descriptors_distances <- compute_distances(descriptors, normalized_descriptors_database, label_movements, 
                                                 labelled_max_distance) # distance between 1 vector of window descriptors 
                                                                        # and all the database descriptors vectors

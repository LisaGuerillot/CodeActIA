...
immobility_threshold <- 0.67
immobility_length_threshold <- 1
no_labelled_threshold <- 0.25
...
for (k in 1 : min(length(ind_beg_seg_filter), length(ind_end_seg_filter))){
  ...
  for (i in seq(1, length(left_segment_data), by=width_sliding_window/2)){
    ...
    if (!(TRUE%in%is.na(descriptors))){
      ...
      forest <- randomForest(label_movements ~ ., train, ntree = tree_number, importance = TRUE)
      ...
      label_confidence_level <- c(label, confidence_level)
...
##########################
#We display the importance of descriptors in the classification by doing the mean between the gains in purity and the gains in predictive performance.
MeanDecreaseAccuracy <- as.numeric(forest$importance[, 'MeanDecreaseAccuracy']/max(forest$importance[, 'MeanDecreaseAccuracy']))
MeanDecreaseGini <- as.numeric(forest$importance[, 'MeanDecreaseGini']/max(forest$importance[, 'MeanDecreaseGini']))

weight <- c()
descr <- c("left_mean", "right_mean", "left_median", "right_median", "left_var", "right_var", "left_Q1", "right_Q1", "left_Q3", "right_Q3", "left_kurto", "right_kurto", "left_skew", "right_skew")
for (i in 1:14){
  val <- (MeanDecreaseAccuracy[i] + MeanDecreaseGini[i])/2
  weight <- c(weight, val)
}
descriptors_weight <- data.frame(descr, weight)
